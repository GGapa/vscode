...
void dfs(int u)
{
	...
	if (u<=n)
	{
		...
		for (auto v:E[u])
		{
			...
			{
				static info vl[2],vr[2];
				rep(i,0,1) vl[i]=gen(add(mp[u][E[v][0]],i*V));
				rep(i,0,1) vr[i]=gen(add(mp[u][E[v].back()],i*V));
				rep(cl,0,1) rep(cr,0,1)
				{
					static info ng[2][2];
					rep(i,0,1) rep(j,0,1) ng[i][j]=up(g[v][i][j],cl+cr);
					nf0=nf0+(ng[0][0]*(vl[cl]*vr[cr]));
					nf0=nf0+(ng[0][1]*vl[cl]);
					nf0=nf0+(ng[1][0]*vr[cr]);
					nf0=nf0+ng[1][1];
					nf1=nf1+ng[0][0];
				}
			}
