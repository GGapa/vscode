...
{
	rep(i,0,P) rep(j,0,K) inc(x.a[i][j],y.a[i][j]);
	return x;
}
inline info operator * (const info &x,const info &y)
{
	info z;
	rep(i,0,P) rep(s,0,K) if (x.a[i][s])
	{
		int &v=x.a[i][s];
		rep(j,0,P-i) rep(t,0,K-s) if (y.a[j][t])
		{
		...
		rep(i,1,(int)E[u].size()-1)
		{
			...
			rep(fi,0,1)
			{
				...
				rep(c,0,1)
				{
					...
					nh0=nh0+(h[fi][0]*f[v][0]*gen(nw));
					nh0=nh0+(h[fi][1]*f[v][0]);
					nh1=nh1+(h[fi][0]*f[v][1]);
				}
				h[fi][0]=nh0;
				h[fi][1]=nh1;
			}
